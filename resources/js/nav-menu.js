...
document.querySelector('.nav-container').addEventListener('mouseover',function(e){        
    if(e.target.classList.contains('toggle')){
                ...
                document.querySelector('.overlay').classList.add('active');
                linktitle.nextElementSibling.classList.add('show');
            }
});
...
for(var j=0;j<toggles.length;j++){
    ...
    //bebinam chera in ba inke roye title khodeshe, digivlub ro pass mide
        /* if(e.target!=document.getElementsByClassName('submenu')[j]){
            document.getElementsByClassName('submenu')[j].classList.remove('show');
            document.querySelector('.overlay').classList.remove('active');
        } */
